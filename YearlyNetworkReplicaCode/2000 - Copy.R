...
indv2000$sy<-as.numeric(sapply(indv2000$startYear, min))
...
indv2000$experience <- sapply(indv2000$startYear, function(x) str_count(x,"\\,"))#_experience####
...
for(nconst in link2000$nconst){
  ...
  tmp<-rbind(tmp,data.frame(nconst, 
                            setNames(
                              ...
                              link2000[link2000$nconst %in% str_trim(names(c)),], mean)
                              , c("tconst", "team_quality"))
                            )
             )} # V(tmp1)$name[c] 
indv2000<-merge(indv2000,aggregate(team_quality ~ nconst, tmp[tmp$tconst %in% rtng2000$tconst,], mean),by="nconst",all.x=TRUE) 
...
for(i in na.omit(xx)){link2000[,i]<-str_count(link2000$genres, i)}
indv2000  <- indv2000[,-which(names(indv2000)%in%xx)]
for(i in na.omit(xx)){indv2000  <- merge(x = indv2000, y = 
...
#_average_confusion####
rtng2000$average_confusion  <-aggregate(average_confusion ~ tconst, merge(link2000[,c("nconst","tconst")], indv2000, by="nconst", all.x=T), mean)$average_confusion #link2000$category %in% c("director")
#_experience####
rtng2000$experience         <-aggregate(experience ~ tconst, merge(link2000[,c("nconst","tconst")], indv2000, by="nconst", all.x=T), mean)$experience #link2000$category %in% c("director")
#_genre_confusion####
rtng2000                    <-merge(x = rtng2000, y = gnrs[, c("tconst", "gConf")], by = "tconst", all.x=TRUE)
#_so####
rtng2000$orientation        <-aggregate(orientation ~ tconst, merge(link2000[,c("nconst","tconst")], indv2000, by="nconst", all.x=T), mean)$orientation
#_role_consolidation####
rtng2000$role_consolidation <-aggregate(role_consolidation ~ tconst, merge(link2000[,c("nconst","tconst")], indv2000, by="nconst", all.x=T), mean)$role_consolidation
#_awards####
rtng2000$awards             <-aggregate(awards ~ tconst, merge(link2000[,c("nconst","tconst")],indv2000, by="nconst", all.x=T), mean)$awards 
#_quality####
rtng2000$quality<-aggregate(quality ~ tconst, link2000, mean)$quality
#_focus on roles####
rtng2000$focus_genres       <-aggregate(focus_genres ~ tconst, merge(link2000[,c("nconst","tconst")],indv2000, by="nconst", all.x=T), mean)$focus_genres
#_focus on genres####
rtng2000$focus_roles        <-aggregate(focus_roles ~ tconst, merge(link2000[,c("nconst","tconst")],indv2000, by="nconst", all.x=T), mean)$focus_roles
#_brokerage####
rtng2000$brokerage          <-aggregate(brokerage ~ tconst, merge(link2000[,c("nconst","tconst")],indv2000, by="nconst", all.x=T), mean)$brokerage#rtng2000$brokerage_smp       <-na.omit(aggregate(brokerage_smp ~ tconst, merge(link2000[,c("nconst","tconst")],indv2000, by="nconst", all.x=T), mean)$brokerage_smp)
#_coreness####
rtng2000$coreness           <-aggregate(coreness ~ tconst, link2000, mean)$coreness#setNames(aggregate(coreness ~ tconst, link2000, mean), c("tconst", "team_coreness"))$team_coreness
#!newcomer# rtng2000$newcomer<-NA # for (tconst in merge(link2000[!,c("nconst","tconst")])){#   rtng2000[rtng2000$tconst=="rtng2000", "newcomer"]<-#   ifelse(aggregate(newcomer ~ tconst, merge(link2000[!,c("nconst","tconst")], indv2000, by="nconst", all.x=T)$newcomer, mean) #          ==1#          , 1, NA)}####
